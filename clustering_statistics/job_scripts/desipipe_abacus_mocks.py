...
def postprocess_stats(tracer='LRG', version='abacus-2ndgen-complete', imocks=[0], stats_dir=Path(os.getenv('SCRATCH')) / 'measurements', postprocess=['combine_regions'], **kwargs):
    ...
    zranges = tools.propose_fiducial('zranges', tracer)
    get_stats_fn = functools.partial(tools.get_stats_fn, stats_dir=stats_dir)
    options = dict(catalog=dict(version=version, tracer=tracer, zrange=zranges, imock=0), imocks=imocks, combine_regions={'stats': ['mesh2_spectrum', 'mesh3_spectrum', 'window_mesh2_spectrum', 'covariance_mesh2_spectrum', 'window_mesh3_spectrum']}, mesh2_spectrum={'cut': True}, window_mesh2_spectrum={'cut': True})
