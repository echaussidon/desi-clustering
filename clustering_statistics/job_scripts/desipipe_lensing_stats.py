"""
Script to create and spawn desipipe tasks to compute clustering measurements for DESI x CMB KP.
To create and spawn the tasks on NERSC, use the following commands:
```bash
source /global/common/software/desi/users/adematti/cosmodesi_environment.sh new
python desipipe_lensing_stats.py          # create the list of tasks
desipipe tasks  -q lensing_stats          # check the list of tasks
desipipe spawn  -q lensing_stats --spawn  # spawn the jobs
desipipe queues -q lensing_stats          # check the queue
```
"""
import os
...
queue = Queue('lensing_stats')
queue.clear(kill=False)

output, error = 'slurm_outputs/lensing_stats/slurm-%j.out', 'slurm_outputs/lensing_stats/slurm-%j.err'
kwargs = {}
...
def run_stats(tracer='LRG', version='abacus-2ndgen-complete', imocks=[0], stats_dir=Path(os.getenv('SCRATCH')) / 'measurements', stats=['mesh2_spectrum'], ibatch=None, **kwargs):
    ...
    for imock in imocks:
        regions = ['NGC','SGC','GAL040','GAL060','N','SnoDES','DES','ACT_DR6','PLANCK_PR4']
